...
class Temporal(nn.Module):
    ...
    def forward(self, tweets,  tokenizer, transformer, device):
        for idx, i in tweets:
            if idx == 0:
                current_tweet = "First tweet: " + i['text']
                cls_embedding_input = self.get_embedding(current_tweet, tokenizer, transformer, device)
                tensor_of_embeds = cls_embedding_input.unsqueeze(0)
            else:
                time_lapse = (user[idx]['created_at'] - user[idx-1]['created_at'])
                current_tweet = "After " + str(time_lapse) + ": " + i['text']
                cls_embedding_input = self.get_embedding(current_tweet, tokenizer, transformer, device)
